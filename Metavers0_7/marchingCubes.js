console.log("./marchingCubes.js","loaded")
...
class Terrain{
	...
	constructor(chunkPosition = undefined) {
		...
		this.mesh = this.genMesh(this.generation);
	...
	genMesh(generation){
		...
		const x0 = this.chunkPosition[0] * chksize;
		const y0 = this.chunkPosition[1] * chksize;
		const z0 = this.chunkPosition[2] * chksize;

		const trianglesArray = marchingChunk(this.chunkPosition,generation);
		...
		mesh.name = "Terrain/" + this.chunkPosition;
	...
	collisionInit(){
		const chk = Chunk.data.list[ this.chunkPosition ];

		if (chk.group.children[0].geometry.attributes.position.count > 0) {
			chk.octree.fromGraphNode( chk.group );
			newCollisionArea(worldOctree,chk.octree);
		...
		/*
		const group = Chunk.data.list[ this.chunkPosition ].group;
	...
	meshUpdate(newGen){
		...
		//temp
		/*
		if (this.mesh.geometry.boundingSphere.radius === 0) {
			this.mesh.geometry.boundingSphere.set(new THREE.Vector3(0,0,0), Chunk.data.chunkSize);
		}*/
