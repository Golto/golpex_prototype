...
/*
worldOctree.bounds = { min : new THREE.Vector3(), max : new THREE.Vector3()}
worldOctree.box = { min : new THREE.Vector3(), max : new THREE.Vector3()}
worldOctree.bounds.min.set(0,0,0);
worldOctree.bounds.max.set(0,0,0);
worldOctree.box.min.set(0,0,0);
worldOctree.box.max.set(0,0,0);*/

function newCollisionArea(parentOctree,childOctree){

	if(parentOctree.bounds && childOctree.bounds){
		parentOctree.bounds.min.x = Math.min(parentOctree.bounds.min.x,childOctree.bounds.min.x);
		parentOctree.bounds.min.y = Math.min(parentOctree.bounds.min.y,childOctree.bounds.min.y);
		parentOctree.bounds.min.z = Math.min(parentOctree.bounds.min.z,childOctree.bounds.min.z);

		parentOctree.bounds.max.x = Math.max(parentOctree.bounds.max.x,childOctree.bounds.max.x);
		parentOctree.bounds.max.y = Math.max(parentOctree.bounds.max.y,childOctree.bounds.max.y);
		parentOctree.bounds.max.z = Math.max(parentOctree.bounds.max.z,childOctree.bounds.max.z);
	}
	
	if(parentOctree.box && childOctree.box){
		parentOctree.box.min.x = Math.min(parentOctree.box.min.x,childOctree.box.min.x);
		parentOctree.box.min.y = Math.min(parentOctree.box.min.y,childOctree.box.min.y);
		parentOctree.box.min.z = Math.min(parentOctree.box.min.z,childOctree.box.min.z);

		parentOctree.box.max.x = Math.max(parentOctree.box.max.x,childOctree.box.max.x);
		parentOctree.box.max.y = Math.max(parentOctree.box.max.y,childOctree.box.max.y);
		parentOctree.box.max.z = Math.max(parentOctree.box.max.z,childOctree.box.max.z);
	}
	

	parentOctree.subTrees.push(childOctree);
}

function deleteCollisionArea(parentOctree,childOctree){
	const i = 0;//indexFrom(childOctree)
	parentOctree.subTrees.splice(i,1);
}
...
function throwBall() {
				...
				const impulse = 5 + 12 * ( 1 - Math.exp( ( mouseTime - performance.now() ) * 0.001 ) );
