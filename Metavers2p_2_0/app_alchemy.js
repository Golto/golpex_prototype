...
const container = document.getElementById('golpexAlchemy').getElementsByClassName('container')[0];
...
handler.outputList = handler.addOutput();
handler.outputList.element.classList.add('list');
...
recipe : ${input}
### response :`
	let output = await bp.promptIteratorLLM(prompt, 1);
...
handler.output.printLog(`Modèle recommandé : ${bp.MODELS.GUANACO_33B}\nModèle utilisé : ${bp.CURRENT_MODELS.LLM}`)
AlchemyList()
