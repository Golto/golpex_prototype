...
exitBtn.onclick = () => { GO_TO_RENDERER('main') };
...
handler.output.element.classList.remove('output');
handler.output.element.classList.add('output2');
...
const lds = document.getElementById('golpexMath-lds');
...
### response :
GOLPEX BOT : `;
	let res = await bp.promptIteratorLLM(prompt, MAX_ITERATION);
	let output = bp.outputExtractor(prompt, res);
...
send("System",`Modèle recommandé : ${bp.models.guanaco33b}\nModèle utilisé : ${bp.currentModels.LLM}`);
...
exitBtn.appendChild(createElement.fa('right-from-bracket'));
/*
...
text = "${input}"
### response : `;
	let res = await bp.promptIteratorLLM(prompt, 4);
	let output = bp.outputExtractor(prompt, res);
