...
async function promptIteratorLLM(prompt, max, isIssueFixed = true){
	...
	for (let i = 0; i < max; i++) {
		...
		console.log(outputExtractor(prompt, result));
		...
		const cleanList = ['END #', 'User :', 'USER :', 'USER:', '#ars', 'package com', '#this', '#<END', '###', '<END>', '# 19', 'Tags:', '# END', '# 1.'];
		for (let cleaner of cleanList) {
			result = result.replaceAll(cleaner, '<END>');
		}
