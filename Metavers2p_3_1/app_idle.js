...
let app, promptInput, displayOutput, elementsOutput, lds;
...
class Inventory {
	...
	get toDisplay() {
		...
		for(let [item, value] of this.items) {
			//console.log(item,handler.output.element.getElementsByClassName('element')[item])
			handler.output.element.getElementsByClassName('element')[item].innerText = `${item} : ${inventory.items.get(item)}`;
		}
...
Write ${STOP_TOKEN} after format is complete.
### input :
${input}
### response : `
...
Write ${STOP_TOKEN} after finishing answering questions.
### input :
${input}
### response : `
...
async function searchCraft(input) {
	...
	if (craft) {
		...
		displayOutput.innerText = `${craft.toDisplay}`//HANDLER 
		return 1
	}
	...
	if (res.isCorrect) {
		...
		elementsOutput.innerText = `${craft.toDisplay}` //HANDLER
		//
		/*
		const icon = await bp.promptText2Img(`${craft.item}, low poly`);
		icon.style.width = '50px';
		icon.style.height = '50px';
		const idleElements = handler.output.element.getElementsByClassName('element');
...
function applyCraft(craft) {
	...
	const idleElements = elementsOutput.getElementsByClassName('element');
...
function addElement(item) {
	...
	element.addEventListener('click', function(){
		...
		displayOutput.innerText = craft.toDisplay;// HANDLER DISPLAY
		applyCraft(craft);
	...
	elementsOutput.appendChild(element);
}
...
RENDERERS_INIT.set('appIdle', init);