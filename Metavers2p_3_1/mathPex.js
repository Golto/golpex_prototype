...
class StructureBuilder {
	constructor(name) {
		...
		this.__constructor = (varName, array) => {
			...
			struct.clone = () => {
				return this.__constructor('TEMP', [...struct.array])
			};
	...
	execObjectLine(input) {
		...
		if (Structure.VARIABLES.get(res.name)) {
			//console.error(`${res.name} existe déjà : ${Structure.VARIABLES.get(res.name)}`);
			throw new Error(`${res.name} existe déjà : ${Structure.VARIABLES.get(res.name)}`);
			return 0
		}

		const object = this.__constructor(res.name, res.args.map( e => Number(e)));
...
function readObjectLine(line) {
	...
	if (a[0] === 'object') {
		...
		return {role : 'object', name : OBJECT_NAME, args : ARGS }
	}
}
...
a = polynom.__constructor('A', [1,2,3,4]);
b = polynom.__constructor('B', [2,3,4,5]);
*/
...
function newClass(name) {
	// existe déjà
	const _class = StructureBuilder.classes.get(name);
...
function deleteClass(name) {
	StructureBuilder.classes.delete(name)
}

function addNewMethod(className, code) {
	const _class = StructureBuilder.classes.get(className);
	_class.execMethodCode(code);
}

function deleteMethod(className, name) {
	const _class = StructureBuilder.classes.get(className);
	_class.methods.delete(name);
}

function addNewObject(className, code) {
	const _class = StructureBuilder.classes.get(className);
	_class.execObjectLine(code);
}

function deleteObject(className, name) {
	const _class = StructureBuilder.classes.get(className);
	_class.objects.delete(name);
	Structure.VARIABLES.delete(name);
}
...
export {
	...
	newClass, deleteClass, addNewMethod, deleteMethod, addNewObject, deleteObject,
	readMethodCode, readObjectLine,
}