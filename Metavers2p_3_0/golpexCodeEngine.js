...
class Program {
	...
	exec(input) { // temp
		...
		const value = this.variables.get(lexed.arguments[0]).clone()
		...
		return {result, value , variable : lexed.arguments[0]}
	}
}
...
Program.getOperation = (operationName, {variables, args, toVariable}) => {
	...
	if (!operation) {
		...
		return (a, b) => {
			...
			//console.log(a.toString(), b.toString(), operationName, a[operationName](b))
			b = ['real', 'integer'].includes(b.instanceof) ? b.get(0) : b
	...
	return (a, b) => {
		...
		if (['real', 'integer'].includes(a.instanceof) && ['real', 'integer'].includes(b.instanceof)) {
			const value = operation(a.get(0), b.get(0))
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}

		if (['real', 'integer'].includes(a.instanceof) && Number.isFinite(b)) {
			const value = operation(a.get(0), b)
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}

		if (Number.isFinite(a) && ['real', 'integer'].includes(b.instanceof)) {
			const value = operation(a, b.get(0))
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}
...
/*
Program.operations[name.toUpperCase()] = (a, b) => a[name](b)
*/
...
Program.getFunction = (_functionName, {variables, args, toVariable}) => {
	...
	if (!_function) {
		...
		return (x, i) => {
			//console.log(x.toString(), i.toString())
			if (i) {
				i = ['real', 'integer'].includes(i.instanceof) ? i.get(0) : i
			}
	...
	return (x, i) => {
		...
		if (['real', 'integer'].includes(x.instanceof) && !i && !Number.isFinite(i)) {
			const value = _function(x.get(0))
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}

		if (['real', 'integer'].includes(x.instanceof) && ['real', 'integer'].includes(i.instanceof)) {
			const value = _function(x.get(0), i.get(0))
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}

		if (['real', 'integer'].includes(x.instanceof) && Number.isFinite(i)) {
			const value = _function(x.get(0), i)
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}

		if (Number.isFinite(x) && ['real', 'integer'].includes(i.instanceof)) {
			const value = _function(x, i.get(0))
			return Program.builtinClass['real']._constructor('__TEMP__',[value])
		}
...
Program.functions = {
	...
	// entre 0 et 1
	RANDOM : (x,i) => Math.random(),
	RANDINT : (x,i) => {
		return Math.floor(x + Math.random() * ( i - x ) )
	},
...
Program.getInstruction = (instructionName, {variables, args, toVariable}) => {
	...
	return instruction(variables, args, toVariable)
}
...
Program.getVariable = (arg, variables, toNumber = false) => {
	...
	if (toNumber) {
		console.log(value)
		return ['real', 'integer'].includes(value.instanceof) ? value.get(0) : value
	}
	return value
}
...
Program.instructions.slice = (variables, args) => {
	let _this = Program.getVariable(args[0], variables);
	let start = Program.getVariable(args[1], variables, true);
	let end = Program.getVariable(args[2], variables, true);
...
Program.instructions.get = (variables, args, toVariable) => { // pas test
	let _this = Program.getVariable(args[0], variables);
	let index = Program.getVariable(args[1], variables, true);
...
Program.instructions.map = (variables, args) => {
	...
	if (arg) {
		const i = Program.getVariable(arg, variables);
		//console.log(arg, x)
		_this.map(x => _function(x,i));
	} else {
		_this.map(_function);
	}
	return _this
...
Program.instructions.id = (variables, args, toVariable) => { // pas test
	let n = Program.getVariable(args[0], variables);

	if (args[1]) {
		if (args[1].toLowerCase() === 'array') {
			const array = StructObject.id(n).array.map( i => Program.builtinClass['integer']._constructor('__TEMP__', [i]) )
			const value = Program.builtinClass['array']._constructor('__TEMP__', array );
			variables.set(toVariable, value);
			return value
		}
	}
	
	const value = StructObject.id(n);
	variables.set(toVariable, value);
	return value
}

Program.instructions.join = (variables, args, toVariable) => { // pas test
	let a = Program.getVariable(args[0], variables);
	let b = Program.getVariable(args[1], variables);

	if (args[2]) {
		if (args[2].toLowerCase() === 'array') {
			const value = Program.builtinClass['array']._constructor('__TEMP__', StructObject.join(a, b).array );
			variables.set(toVariable, value);
			return value
		}
	}
	
	const value = StructObject.join(a, b);
	variables.set(toVariable, value);
	return value
}
...
/*
			...
			structObject.index = () => {
				const array = Array.from(structObject.array.keys()).map( r => Program.builtinClass['real']._constructor('__TEMP__', [r]) );
				return Program.builtinClass['array']._constructor('__TEMP__', array )
			}

			structObject.length = () => {
				return Program.builtinClass['integer']._constructor('__TEMP__', [structObject.array.length] )
			}
			*/
/*
			let temp = () => {
				const array = structObject.index().array.map( r => Program.builtinClass['real']._constructor('__TEMP__', [r]) );
				console.log(array)
				return Program.builtinClass['array']._constructor('__TEMP__', array )
			}*/

			const temp = structObject.index().array

			structObject.index = () => {
				const array = temp.map( r => Program.builtinClass['real']._constructor('__TEMP__', [r]) );
				//console.log(array)
				return Program.builtinClass['array']._constructor('__TEMP__', array )
			}
...
système de dérivation formelle
fractions continues
algèbre tensorielle
