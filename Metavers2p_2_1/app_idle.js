...
const container = document.getElementById('golpexIdle').getElementsByClassName('container')[0];
...
handler.output.element.classList.add('items');

handler.craftDisplay = handler.addOutput();
...
${input}
### response :`;
	let output = await bp.promptIteratorLLM(prompt, 8, false);
...
${input}
### response :  `;
	let output = await bp.promptIteratorLLM(prompt, 3, true);
