...
multiContainer.classList.add('multi');
multiContainer.classList.add('row');
multiContainer.style.height = '100%'
container.appendChild(multiContainer);

//---
const structureTab = document.createElement('div');
structureTab.classList.add('structure');
structureTab.style.width = '0%'
structureTab.style.background = 'rgba(244, 244, 244, 0.1)'
multiContainer.appendChild(structureTab);



const chatBotTab = document.createElement('div');
chatBotTab.classList.add('chatbot');
chatBotTab.classList.add('column');
chatBotTab.style.width = '100%'
chatBotTab.style.height = 'calc(100% - 48px)';
multiContainer.appendChild(chatBotTab);
...
/*
	if (input === 'struct') {
		structureTab.classList.add('full-width');
		chatBotTab.classList.add('empty-width');
		structureTab.classList.remove('empty-width');
		chatBotTab.classList.remove('full-width');
	...
	if (input === 'chat') {
		structureTab.classList.add('empty-width');
		chatBotTab.classList.add('full-width');
		structureTab.classList.remove('full-width');
		chatBotTab.classList.remove('empty-width');
...
/*
		...
		handler.output.element.appendChild(
			createElement.desmos(desmosRedirect.noise)
		)
/*
		handler.output.element.appendChild(
		...
		structureTab.appendChild(createElement.youtube('WoqPAbNbkB8'))
...
Possible command :
- "graph" args : equation
- "code" args : task to complete
- "info" args : info to get
- "wikipedia" args : topic
- "structure" args : className
...
L.observe('listValue', function() {
  console.log(L.listValue);
	...
	classObjectObject : (name) => {
		...
		objectAppearence.value = 'NAME = this';
	...
	classObject : (name) => {
		...
		methodAdd.onclick = () => {methodsContainer.appendChild(createElement.classMethodObject())};
		objectAdd.onclick = () => {objectsContainer.appendChild(createElement.classObjectObject())};
...
let desmosRedirect = {
	void : 'mjnwlblznb',
	structureDisplay : 'dt9xgsrxtc',
	noise : 'j6oykvyg1v',
}
// https://www.desmos.com/calculator/dt9xgsrxtc
